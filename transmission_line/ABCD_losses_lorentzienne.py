...
def fit_lorentzian(_ws, _sqr_abs_z, _peaks, _heights, threshold=0.1, _n_point_fit=10 ** 4):
    _list_guess_width = []
    _wps = []
    _qs = []
    for p, he in zip(_peaks, _heights):
        ind = p
...
peaks, properties = find_peaks(sqr_abs_z, height=0)
peaks_plot = ws[np.array(peaks)]

fig, ax = plt.subplots(3, 1, figsize=(8, 6))

wps, qs, ind_half_widths = fit_lorentzian(ws, sqr_abs_z, peaks, properties['peak_heights'])
